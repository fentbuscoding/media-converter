...
export class MediaDownloader {
    ...
    async getYouTubeVideoInfo(videoId) {
        try {
            ...
            // Fallback to noembed
            const fallbackResponse = await fetch(`https://noembed.com/embed?url=https://www.youtube.com/watch?v=${videoId}`);
    ...
    async downloadYouTubeVideo(videoId, quality = 'highest', format = 'mp4') {
        try {
            ...
            throw new Error(`Download failed: ${error.message}. Make sure the API server is running on http://localhost:3000`);
        }
    }
    ...
    async downloadYouTubeAudio(videoId, format = 'mp3') {
        try {
            ...
            throw new Error(`Audio download failed: ${error.message}. Make sure the API server is running on http://localhost:3000`);
        }
    }
    ...
    async getInstagramMediaInfo(postUrl) {
        try {
            // Try local API first
            const response = await fetch(`${this.apiUrl}/instagram/info?url=${encodeURIComponent(postUrl)}`);
            if (response.ok) {
                const data = await response.json();
                return data.data;
            }
    ...
    async downloadInstagramMedia(postUrl, quality = 'highest') {
        try {
            ...
            throw new Error(`Instagram download failed: ${error.message}. Make sure the API server is running on http://localhost:3000`);
        }
    }
