    ...
    try {
        ...
        process.stdout.on('data', (data) => {
            ...
            if (progressMatch || speedMatch || etaMatch) {
                downloadProgress.set(downloadId, {
                    status: 'downloading',
                    progress: progressMatch ? parseFloat(progressMatch[1]) : downloadProgress.get(downloadId).progress,
    ...
    try {
        ...
        process.on('close', (code) => {
            ...
            if (finalFile) {
                ...
                console.log(`✅ [${downloadId}] Downloaded audio: ${finalFilename} (${formatBytes(stats.size)})`);
...
app.get('/api/instagram/info', async (req, res) => {
    try {
        ...
        const shortcode = shortcodeMatch ? (shortcodeMatch[1] || shortcodeMatch[2] || shortcodeMatch[3]) : null;
